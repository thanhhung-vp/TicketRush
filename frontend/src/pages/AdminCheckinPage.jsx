...
export default function AdminCheckinPage() {
  ...
  const [result, setResult]  = useState(null); // { ok, message, ticket, error }
  ...
  const handleScan = async (e) => {
    ...
    try {
      ...
      setResult({ ok: false, error: data?.error || 'Lỗi không xác định', ticket: data?.ticket });
    } finally {
      setLoading(false);
      setQr('');
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Kiểm soát vào cửa (Check-in)</h1>

      {/* Stats */}
      {stats && (
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[
            { label: 'Tổng vé', value: stats.total_tickets, color: 'text-gray-700' },
            { label: 'Đã vào', value: stats.checked_in, color: 'text-green-600' },
            { label: 'Chưa vào', value: stats.remaining, color: 'text-orange-500' },
          ].map(s => (
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6 shadow-sm">
        <h2 className="text-base font-semibold text-gray-800 mb-4">Quét mã QR</h2>
        <form onSubmit={handleScan} className="flex gap-3">
          <input
            ...
            placeholder="Dán hoặc quét mã QR vào đây..."
        ...
        <p className="text-xs text-gray-400 mt-2">
          Sử dụng máy quét QR hoặc dán nội dung mã QR từ vé điện tử
        </p>
      </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className={`font-semibold ${
                ...
                {result.ok ? 'Check-in thành công!' : result.error}
              </p>
              {result.ticket && (
                <div className="mt-2 text-sm space-y-0.5 text-gray-600">
                  <p><span className="font-medium">Khách:</span> {result.ticket.full_name} ({result.ticket.email})</p>
                  <p><span className="font-medium">Ghế:</span> {result.ticket.zone_name} — {result.ticket.seat_label}</p>
                  {result.ticket.checked_in_at && (
                    <p><span className="font-medium">Đã quét lúc:</span> {new Date(result.ticket.checked_in_at).toLocaleTimeString('vi-VN')}</p>
                  )}
                </div>
              )}
            </div>
      ...
      {history.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100">
            <h2 className="font-semibold text-gray-800">Lịch sử check-in ({history.length})</h2>
          </div>
            ...
            {history.map(t => (
              <div key={t.id} className="px-6 py-3 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400">{t.zone_name} — {t.seat_label}</p>
                </div>
                <p className="text-xs text-gray-400">
                  {new Date(t.checked_in_at).toLocaleTimeString('vi-VN')}
                </p>
              </div>
            ))}
          </div>
