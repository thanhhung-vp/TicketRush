...
export default function AdminEventPage() {
    ...
    if (!isNew) {
      eventService.getById(id)
        .then(ev => {
          ...
          setForm({
            ...rest,
            event_date: rest.event_date ? new Date(rest.event_date).toISOString().slice(0, 16) : '',
  ...
  const saveEvent = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Không thể lưu sự kiện.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const saveLayout = async (layoutZones, stages, canvas) => {
    ...
    try {
      ...
      setSuccess('Đã lưu sơ đồ sự kiện.');
    } catch (err) {
      setError(err.response?.data?.error || 'Không thể lưu sơ đồ sự kiện.');
    } finally {
  ...
  const deleteEvent = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Không thể xóa sự kiện.');
      setDeleting(false);
    }
  };
  ...
  return (
    <PageContainer maxWidth="max-w-3xl">
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button
            ...
            className="text-sm text-label-secondary transition hover:text-label-primary"
          >
            Quay lại
          </button>
        ...
        {!isNew && form.status === 'draft' && (
          <Button variant="destructive" onClick={deleteEvent} disabled={deleting} className="text-sm">
            {deleting ? 'Đang xóa...' : 'Xóa sự kiện'}
          </Button>
        )}
      </div>

      <form onSubmit={saveEvent} className="mb-8 space-y-5 rounded-2xl border border-separator bg-surface p-6 shadow-1">
        <h2 className="border-b border-separator pb-3 font-semibold text-label-primary">Thông tin sự kiện</h2>

        {error && <Alert type="error">{error}</Alert>}
        {success && <Alert type="success">{success}</Alert>}

        <FormField label="Tên sự kiện *">
          <Input type="text" required value={form.title} onChange={set('title')} placeholder="VD: Đêm nhạc Acoustic 2026" />
        </FormField>
        ...
        <FormField label="Mô tả">
          <textarea
            ...
            className="w-full resize-none rounded-xl border border-transparent bg-fill-tertiary px-3 py-2.5 text-sm text-label-primary placeholder:text-label-tertiary transition focus:border-info focus:outline-none focus:shadow-focus"
          />
        </FormField>

        <FormField label="Địa điểm *">
          <Input type="text" required value={form.venue} onChange={set('venue')} placeholder="VD: Nhà hát Lớn, Hà Nội" />
        </FormField>

        <FormField label="Thời gian *">
          <Input type="datetime-local" required value={form.event_date} onChange={set('event_date')} />
        </FormField>
        ...
        <FormField label="Ảnh poster">
          ...
          {form.poster_url && (
            <img
              src={form.poster_url}
              alt="Poster preview"
              className="mt-3 h-32 rounded-xl border border-separator object-cover"
            />
          )}
        </FormField>
      ...
      {false && !isNew && (
        <div className="space-y-5 rounded-2xl border border-separator bg-surface p-6 shadow-1">
          <h2 className="border-b border-separator pb-3 font-semibold text-label-primary">Khu vực ghế ngồi</h2>

          {zones.length === 0 ? (
            <p className="text-sm text-label-secondary">Chưa có khu vực nào. Thêm khu bên dưới.</p>
          ) : (
            <div className="space-y-2">
              {zones.map(z => (
                <div key={z.id} className="flex items-center justify-between rounded-xl bg-fill-tertiary px-4 py-3">
                  <div className="flex items-center gap-3">
                    <span className="h-4 w-4 flex-shrink-0 rounded-sm" style={{ backgroundColor: z.color }} />
                    <div>
                      <p className="text-sm font-medium">{z.name}</p>
                      <p className="text-xs text-label-secondary">
                        {z.rows} hàng x {z.cols} ghế = {z.rows * z.cols} ghế
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <span className="text-sm font-medium text-info">{formatVND(z.price)}</span>
                    <button onClick={() => deleteZone(z.id)} className="text-sm text-danger transition hover:underline">
                      Xóa
                    </button>
                  </div>
                </div>
          ...
          <form onSubmit={addZone} className="space-y-4 rounded-xl border border-separator p-4">
            <p className="text-sm font-medium text-label-primary">Thêm khu vực mới</p>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <FormField label="Tên khu *">
                <Input type="text" required maxLength={50} value={newZone.name} onChange={setZ('name')} placeholder="VD: Khu VIP" />
              </FormField>
              <FormField label="Màu sắc">
                <input type="color" value={newZone.color} onChange={setZ('color')} className="h-10 w-full cursor-pointer rounded-xl border border-separator bg-fill-tertiary px-2" />
              </FormField>
              <FormField label="Số hàng *">
                <Input type="number" required min={1} max={50} value={newZone.rows} onChange={setZ('rows')} placeholder="VD: 10" />
              </FormField>
              <FormField label="Ghế / hàng *">
                <Input type="number" required min={1} max={50} value={newZone.cols} onChange={setZ('cols')} placeholder="VD: 20" />
              </FormField>
              <div className="sm:col-span-2">
                <FormField label="Giá vé (VND) *">
                  <Input type="number" required min={0} step={1000} value={newZone.price} onChange={setZ('price')} placeholder="VD: 500000" />
                </FormField>
              </div>
            </div>
            <Button type="submit" variant="success">Thêm khu vực</Button>
          </form>
        </div>
      )}
    </PageContainer>
...
function FormField({ label, children }) {
  return (
    <div className="space-y-1">
      <label className="block text-sm font-medium text-label-secondary">{label}</label>
      {children}
    </div>
  );
}
